...
// 🆕 ترجمة رسائل الباك إند الشائعة للعربي — لو النمط مش معروف، بترجع الرسالة الأصلية بدل ما توقف الميزة
function translateApiMessage(rawMessage) {
  if (!rawMessage) return ''

  // نمط: "Please wait X minute(s) before requesting a new verification code."
  const waitMatch = rawMessage.match(/wait\s+(\d+)\s*minute/i)
  if (waitMatch) {
    return `الرجاء الانتظار ${waitMatch[1]} دقيقة قبل طلب رمز جديد`
  }

  // نمط: "Invalid or expired code"
  if (/invalid|expired/i.test(rawMessage)) {
    return 'الرمز غير صحيح أو منتهي الصلاحية'
  }

  // نمط: أخطاء الشبكة العامة من axios
  if (/network error/i.test(rawMessage)) {
    return 'تعذّر الاتصال بالسيرفر، تأكدي من اتصالك بالإنترنت'
  }

  // ما لقينا نمط معروف → رجّعي الرسالة الأصلية بدل ما توقفي الميزة
  return rawMessage
}
...
export default function VerifyOTP() {
  ...
  async function handleResend() {
    ...
    try {
      ...
      setApiError('') // ✅ نشيل أي رسالة خطأ قديمة (زي "الرجاء الانتظار...") لأن المحاولة الحالية نجحت
      ...
      setApiError(translateApiMessage(err.message)) // 🆕 عرض الرسالة مترجمة للعربي
    } finally {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // ✅ عرض الرسالة الحقيقية القادمة من السيرفر مترجمة للعربي بدل رسالة عامة دايماً
      const msg = translateApiMessage(err.message) || 'حدث خطأ، حاول مرة ثانية' // 🆕
      setApiError(msg)
